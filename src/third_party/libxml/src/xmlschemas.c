			   ...
			   xmlNodePtr node)
{
    ...
    * An instance of a datatype that is defined as ï¿½booleanï¿?    * can have the following legal literals {true, false, 1, 0}.
    */
                     ...
                     xmlNodePtr node)
{
    ...
    */
    ...
    if (namespaceName != NULL) {
	/*
	* 1.1 If the namespace [attribute] is present, then its ï¿½actual valueï¿?	* must not match the ï¿½actual valueï¿?of the enclosing <schema>'s
                          ...
                          xmlNodePtr node, xmlSchemaTypeType parentType)
{
    ...
    */
	    ...
	    (type->flags & XML_SCHEMAS_TYPE_GLOBAL))
	{
		...
		* whose base [attribute] must be the same as the ï¿½actual valueï¿?		* of its own name attribute plus target namespace;"
		*/
				  ...
				  xmlSchemaParserCtxtPtr ctxt)
{
	...
	/* (type definition) ... otherwise the type definition ï¿½resolvedï¿?	* to by the ï¿½actual valueï¿?of the type [attribute] ...
	*/
	...
	*/
	...
	if (substHead == NULL) {
	    ...
	    * declaration ï¿½resolvedï¿?to by the ï¿½actual valueï¿?	    * of the substitutionGroup [attribute], if present"
	    */
				 ...
				 xmlSchemaTypePtr type)
{
    ...
    * define the explicit members as the type definitions ï¿½resolvedï¿?    * to by the items in the ï¿½actual valueï¿?of the memberTypes [attribute],
...
inherit_next: {}
    ...
    */
	...
	if (baseType->attributeWildcard != NULL) {
		...
		* of the {namespace constraint} of the ï¿½complete wildcardï¿?		* and of the ï¿½base wildcardï¿? as defined in Attribute
			     ...
			     int subset)
{
	...
	*/
    ...
    /*
    * 2.2.2 D's ï¿½base type definitionï¿?is not the ï¿½ur-type definitionï¿?    * and is validly derived from B given the subset, as defined by this
    * constraint.
    */
			     ...
			     xmlSchemaTypePtr type)
{
    ...
    */
    /* Base type: If the datatype has been ï¿½derivedï¿?by ï¿½restrictionï¿?    * then the Simple Type Definition component from which it is ï¿½derivedï¿?
			   ...
			   xmlSchemaTypePtr type)
{
	...
	*/
	if ((type->contentTypeDef != NULL) &&
		...
		* complex type definition must be a ï¿½valid extensionï¿?		* of the {base type definition}'s particle, as defined
			    ...
			    xmlSchemaParticlePtr b)
{
    ...
    /*
    * SPEC "For an element declaration particle to be a ï¿½valid restrictionï¿?    * of a wildcard particle all of the following must be true:"
    *
    * SPEC (1) "The element declaration's {target namespace} is ï¿½validï¿?    * with respect to the wildcard's {namespace constraint} as defined by
...
xmlSchemaTypeFixupWhitespace(xmlSchemaTypePtr type)
{
    ...
    */
    ...
    /*
    * For all ï¿½atomicï¿?datatypes other than string (and types ï¿½derivedï¿?    * by ï¿½restrictionï¿?from it) the value of whiteSpace is fixed to
    * collapse
    */
    {
	...
	*/
	...
	switch (WXS_PARTICLE_TERM(cur)->type) {
		...
		if (search == 0) {
		    ...
		    * contained model group, recursively) or ï¿½implicitlyï¿?		    * two or more element declaration particles with
		       ...
		       xmlSchemaValPtr y)
{
	...
	* the ï¿½value spaceï¿?of T. */
	/*
	* (2) if datatypes T' and T'' are ï¿½derivedï¿?by ï¿½restrictionï¿?	* from a common atomic ancestor T then the ï¿½value spaceï¿½s of T'
	* and T'' may overlap.
	*/
	if (ptx != pty)
	    return(0);
...
#define NORMALIZE(atype) \
    ...
    */
	...
	/*
	* SPEC (1.2.1) "if {variety} is ï¿½atomicï¿?then the string must ï¿½matchï¿?	* a literal in the ï¿½lexical spaceï¿?of {base type definition}"
	*/
	/*
	...
	* not apply directly; however, the normalization behavior of ï¿½unionï¿?	* types is controlled by the value of whiteSpace on that one of the
	...
	*/
	if ((ret == 0) && (type->flags & XML_SCHEMAS_TYPE_HAS_FACETS)) {
	    ...
	    * the value of whiteSpace on that one of the ï¿½memberTypesï¿?	    * against which the ï¿½unionï¿?is successfully validated.
	    */
...
xmlSchemaVAttributesComplex(xmlSchemaValidCtxtPtr vctxt)
{
	...
	*/
	...
	/*
	* SPEC (cvc-attribute)
	* (3) "The item's ï¿½normalized valueï¿?must be locally ï¿½validï¿?	* with respect to that {type definition} as per
	* String Valid (ï¿?.14.4)."
	*
	...
	*/
	...
	if (fixed) {
	    ...
	    * "For an attribute information item to beï¿½validï¿?	    * with respect to an attribute use its *normalized*
...
character_content:
    ...
    */
	...
	/*
	* cvc-elt (3.3.4) : 5.1.1
	* If the ï¿½actual type definitionï¿?is a ï¿½local type definitionï¿?	* then the canonical lexical representation of the {value constraint}
...
default_psvi:
	...
	*/
	if (WXS_IS_SIMPLE(inode->typeDef)) {
	    ...
	    * (Element) (ï¿?.3.4) did not apply, then the ï¿½normalized valueï¿?	    * must be ï¿½validï¿?with respect to the type definition as defined
